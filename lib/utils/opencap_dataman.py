...
class TRCFile(object):
    ...
    def create_empty_trc(num_frames, data_rate=100.0, camera_rate=100.0, units='mm'):
        """
        Create an empty TRC file with specified parameters
        ...
        camera_rate : float
            Camera sampling rate (Hz)
        units : str
            Units for the data (e.g., 'mm', 'm')
        """
        
        # Create time array
        time = np.linspace(0, (num_frames-1)/data_rate, num_frames)
        
        # Create empty TRC with required attributes
        trc = TRCFile(
            fpath=None,  # No file to read from
            data_rate=data_rate,
            camera_rate=camera_rate,
            num_frames=num_frames,
            num_markers=0,  # Start with 0 markers
            units=units,
    ...
    def create_empty_from_template(template_trc_path):
        """
        Create an empty TRC using metadata from an existing TRC file
        """
        # Read the template to get metadata
        template = TRCFile(template_trc_path)
        
        # Create empty TRC with same metadata but no markers
        empty_trc = TRCFile(
            fpath=None,
            data_rate=template.data_rate,
            camera_rate=template.camera_rate,
            num_frames=template.num_frames,
            num_markers=0,
            units=template.units,
            orig_data_rate=template.orig_data_rate,
            orig_data_start_frame=template.orig_data_start_frame,
            orig_num_frames=template.orig_num_frames,
            marker_names=[],
